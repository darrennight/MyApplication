...
public class BordersPanel extends AbstractOptionPanel implements ViewFactory, OnUpdateListener, OnImageLoadListener,
	...
	public void onCreate( Bitmap bitmap, Bundle options ) {
		...
		try {
			updateArrowBitmap = BitmapFactory.decodeResource( getContext().getBaseContext().getResources(),
					R.drawable.aviary_update_arrow );
		} catch ( Throwable t ) {
		}
	}
	...
	public void onActivate() {
		...
		// mCellWidth = (int) ( getOptionView().findViewById( R.id.aviary_panel
		// ).getHeight() * 0.68 );
	...
	public void onUpdate( PluginService service, Bundle delta ) {
		...
		if ( isActive() && mExternalPacksEnabled ) {
			...
			if ( validDelta( delta ) ) {
				...
				mUpdateDialog = new AlertDialog.Builder( getContext().getBaseContext() )
						.setMessage( R.string.feather_filter_pack_updated ).setNeutralButton( android.R.string.ok, listener )
	...
	protected BaseAdapter createListAdapter( Context context, List<EffectPack> result ) {
		...
		return new ListAdapter( context, R.layout.aviary_frame_item, R.layout.aviary_frame_item_more,
				R.layout.aviary_frame_item_external, R.layout.aviary_frame_item_divider, result );
	}
	 ...
	 *            installed count
	 * @param externalCount
	 *            external items count ( get more and featured )
	 * @param firstValidIndex
	 *            the index of the first valid element
	 */
	private void onEffectListUpdated( List<EffectPack> result, List<EffectPackError> errors, int totalCount, int externalCount,
			int firstValidIndex ) {
		...
		if ( ( null == iapPackageName ) && totalCount < 1 && mExternalPacksEnabled && mPluginType == FeatherIntent.PluginType.TYPE_BORDER ) {
	...
	class ListAdapter extends BaseAdapterExtended<EffectPack> {
		...
		public ListAdapter ( Context context, int defaultResId, int moreResId, int externalResId, int dividerResId,
				List<EffectPack> items ) {
			super();
		...
		public int getItemViewType( int position ) {
			...
			switch ( item.mType ) {
				...
				case GET_MORE:
					return TYPE_GETMORE;
				case LEFT_DIVIDER:
					return TYPE_LEFT_DIVIDER;
				case RIGHT_DIVIDER:
					return TYPE_RIGHT_DIVIDER;
				case PACK_DIVIDER:
					return TYPE_DIVIDER;
				case INTERNAL:
				default:
		...
		public View getView( int position, View convertView, ViewGroup parent ) {
			...
			if ( null == convertView ) {
				...
				if ( type == TYPE_EXTERNAL ) {
					...
					( (ViewHolderExternal) holder ).badgeIcon = convertView.findViewById( R.id.aviary_badge );
					( (ViewHolderExternal) holder ).externalIcon = convertView.findViewById( R.id.aviary_image2 );
				} else {
			...
			if ( type == TYPE_NORMAL && null != item ) {
				...
				if ( type == TYPE_EXTERNAL ) {
					...
					if ( mBadgeService.getIsActive( (String) item.mPackageName ) ) {
					...
					if ( null != plugin ) {
						...
						if( null != getContext() && null != getContext().getBaseContext() ) {
							final String url = PackageManagerUtils.getCDSContentUrl( getContext().getBaseContext() ) + "/" + plugin.getIconUrl();
							executor = createExternalContentCallable( url );
				...
				if ( drawable instanceof PluginDividerDrawable ) {
					...
					PluginDividerDrawable d = new PluginDividerDrawable( getContext().getBaseContext(),
							R.attr.aviaryEffectThumbDividerTextStyle, item.mTitle.toString() );
		...
		protected Callable<Bitmap> createContentCallable( final EffectPack item, int position, final String effectName ) {
			return new BorderThumbnailCallable( mCacheService, (InternalPlugin) item.mPluginRef, effectName, mThumbBitmap,
					mThumbSize );
		}

		protected Callable<Bitmap> createExternalContentCallable( final String iconUrl ) {
			return new ExternalFramesThumbnailCallable( iconUrl, mCacheService, mExternalFolderIcon, getContext().getBaseContext()
					.getResources(), R.drawable.aviary_ic_na );
		}
	}
	...
	public boolean onItemClick( AdapterView<?> parent, View view, int position, long id ) {
		...
		if ( isActive() ) {
			...
			if ( viewType == ListAdapter.TYPE_NORMAL ) {
				...
				if ( null != item ) {
					final ExternalPlugin externalPlugin = (ExternalPlugin) item.mPluginRef;

					// reset the badge icon
					// mBadgeService.markAsRead( externalPlugin.getPackageName() );
					ViewHolderExternal tag = (ViewHolderExternal) view.getTag();
					if( null != tag ) {
						tag.badgeIcon.setVisibility( View.GONE );
						tag.externalIcon.setVisibility( View.VISIBLE );
					}
					
					if ( externalPlugin == null ) return false;

					if ( SystemUtils.getApplicationTotalMemory() >= Constants.APP_MEMORY_SMALL ) {
						
						displayIAPDialog( new IAPUpdater.Builder().setPlugin( externalPlugin ).build() );
						
						// smooth scroll the listview
						int currentScrollX = mHList.getCurrentScrollX();
						
						Rect outRect = new Rect();
						view.getGlobalVisibleRect( outRect );
						
						if( currentScrollX > 0 ) {
							final int delta = mHList.getWidth()/2 - outRect.centerX();
							...
							mHList.post( new Runnable() {
								...
								public void run() {
									int clamped = mHList.computeScroll( delta );
									if ( clamped == 0 ) {
										mHList.smoothScrollBy( -delta, 300 );
									} else {
										mHList.scrollTo( delta );
									}
								}
	...
	static class ExternalFramesThumbnailCallable implements Callable<Bitmap> {
		...
		public Bitmap call() throws Exception {
			...
			try {
				request = cache.requestRemoteBitmap( mUri );
	...
	static class BorderThumbnailCallable implements Callable<Bitmap> {
		...
		public BorderThumbnailCallable ( ImageCacheService cacheService, final InternalPlugin plugin, final String srcUrl,
				final Bitmap bitmap, final int size ) {
			mFinalSize = size;
			mUrl = srcUrl;
			mPlugin = plugin;
	...
	class PluginInstallTask extends AsyncTask<Integer, Void, List<EffectPack>> {
		...
		protected List<EffectPack> doInBackground( Integer... params ) {
			...
			if ( mExternalPacksEnabled ) {
				...
				availablePacks = mPluginService.getAvailable( pluginType, System.currentTimeMillis() );
			} else {
			...
			if ( mExternalPacksEnabled ) {
				...
				if ( size > 0 ) {
					for ( int i = size - 1; i >= 0; i-- ) {
						...
						ExternalPlugin plugin = (ExternalPlugin) PluginFactory.create( getContext().getBaseContext(), pack,
								mPluginType );
						final CharSequence packagename = plugin.getPackageName();
						final CharSequence label = plugin.getPackageLabel();
						final EffectPack effectPack = new EffectPack( EffectPack.EffectPackType.EXTERNAL, packagename, label, null,
								null, PluginService.ERROR_NONE, null, plugin );
			...
			for ( FeatherPack pack : installedPacks ) {
				if ( pack instanceof FeatherInternalPack ) {
					InternalPlugin plugin = (InternalPlugin) PluginFactory
							.create( getContext().getBaseContext(), pack, mPluginType );
					final CharSequence packagename = plugin.getPackageName();
					final CharSequence label = plugin.getPackageLabel();
					...
					if ( null != pluginItems && null != pluginIds ) {
						final EffectPack effectPack = new EffectPack( EffectPack.EffectPackType.INTERNAL, packagename, label,
								pluginItems, pluginIds, status, errorMessage, plugin );
		...
		@Override
		protected void onPostExecute( final List<EffectPack> result ) {
	...
	static class EffectPack {
		...
		public EffectPack ( EffectPackType type, CharSequence packageName, CharSequence pakageTitle,
				List<Pair<String, String>> values, List<Long> ids, int status, String errorMsg, IPlugin plugin ) {
			this( type );
			mPackageName = packageName;
			mStatus = status;
			mTitle = pakageTitle;
	...
	protected class RenderTask extends UserTask<EffectPack, Bitmap, Bitmap> implements OnCancelListener {
		...
		public Bitmap doInBackground( final EffectPack... params ) {
			...
			initFilter( pack, mPosition, mEffect );
