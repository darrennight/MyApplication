...
public class IAPDialog implements OnPageChangeListener, OnClickListener {
	...
	public IAPDialog ( ViewGroup view ) {
		...
		if( null != mView && null != mView.getContext() ) {
			mCdsBaseUrl = PackageManagerUtils.getCDSContentUrl( mView.getContext() );
		} else {
			mCdsBaseUrl = "";
		}
		
		logger.info( "cds base url: " + mCdsBaseUrl );
	...
	private String getRemoteFolder( ExternalPlugin plugin ) {
		...
		if ( null != plugin && null != plugin.getPackageName() ) {
			...
			String folder = null;
			if ( pkg.length >= 2 ) {
				folder = mCdsBaseUrl + "/" + pkg[pkg.length - 2] + "/" + pkg[pkg.length - 1];
				return folder;
			}
		}
		return "";
	...
	public void update( IAPUpdater updater ) {
		...
		if( null != mController ) {
			BadgeService badgeService = mController.getService( BadgeService.class );
			if( null != badgeService ) {
				badgeService.markAsRead( pname );
			}
		}
	...
	private void downloadPlugin( final String name, final int type ) {
		...
		if ( null != mThreadService ) {
			...
			mThreadService.submit( new ExternalPacksTask(), listener, null );
		}
	}
	...
	private void processPlugin() {
		...
		View headView = mView.findViewById( R.id.aviary_head );
		
		if( null != headView ) {
			headView.requestFocus();
			headView.requestFocusFromTouch();
		}
	}
	...
	private void downloadPackIcon( ExternalPlugin plugin ) {
		...
		if ( null != plugin && valid() ) {
			if ( null != mThreadService && null != mIcon ) {
				final String url = mCdsBaseUrl + "/" + plugin.getIconUrl();
			...
			for ( int i = index; i < endIndex; i++ ) {
				...
				if ( i < total ) {
					...
					if ( tag == null || !url.equals( tag ) ) {
						...
						logger.warn( "image already loaded?" );
					}
				}
			}
		}
